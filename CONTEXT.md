...
**Canvas 全景图**:
Canvas 图片节点的全景使用意图，表示该节点应按全景图标记和查看；同一个 metadata 字段出现在非图片生成源节点上时，表示从该节点发起的图片生成应按全景图生成，并作为持久的节点级生成偏好保留到用户手动关闭。它不是 asset 级认证属性，也不表示底层媒体已经被系统验证为合格的 2:1 等距矩形全景文件。
_Avoid_: 全景资产、素材库全景标记、已验证全景文件、图片比例校验结果
...
**Studio 基础资产**:
Studio 短漫剧模块中从剧本分析或用户维护得到的角色、场景和道具条目，是后续镜头一致性和分镜生成的素材锚点。
_Avoid_: 本地素材库 asset、Canvas 节点媒体、镜头图

**待生成基础资产**:
Studio 基础资产中还没有主图的角色、场景或道具，第一次一键生成基础资产图时通常全部属于这个状态。
_Avoid_: 已选主图资产、重新生成全部

**Studio 基础资产生产 prompt**:
Studio 基础资产中用户可见、可编辑、可复制、可保存和可复用的完整生成提示词，是角色、场景或道具图片生成请求的创作输入，并作为基础资产模型的核心字段 `prompt` 保存。
_Avoid_: 隐藏上下文、仅供系统内部使用的描述、metadata 里的附属说明、生成适配层暗中拼接的最终 prompt
...
**Studio 最小生成闭环**:
Studio 短漫剧模块第一阶段必须跑通的真实 relay 链路，从剧本解析到基础资产或镜头候选生成并回填到本地项目数据。
